...
export default function PantryTracker() {
  ...
  const addOrUpdate = () => {
    ...
    if (editId) {
      ...
      save([{ ...form, id: Date.now() + Math.random(), location: activeLocation }, ...items]);
    }
    setForm(defaultItem());
    setAdding(false);
  };
  ...
  return (
    <div style={{ background: "#faf8f4", minHeight: "100vh", fontFamily: "Georgia, serif", color: "#2a2a2a", maxWidth: 520, margin: "0 auto" }}>
        ...
        <div style={{ background: "#fff", margin: 16, borderRadius: 12, padding: 16, border: `2px solid ${loc.color}` }}>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 8, marginBottom: 8 }}>
            <div style={{ gridColumn: "1/-1" }}>
              ...
              <input value={form.name} onChange={e => setForm({ ...form, name: e.target.value })} placeholder="e.g. Greek yogurt" onKeyDown={e => e.key === "Enter" && addOrUpdate()} style={inputStyle} />
            </div>
            ...
            <div><label style={labelStyle}>Use by / best before</label><input type="date" value={form.expiry} onChange={e => setForm({ ...form, expiry: e.target.value })} style={inputStyle} /></div>
      ...
      <div style={{ padding: "8px 16px 80px" }}>
        {sorted.length === 0 && <div style={{ textAlign: "center", padding: "50px 0", color: "#bbb", fontStyle: "italic", fontSize: 14 }}>{search ? "No items match your search." : `Nothing in the ${loc.label.toLowerCase()} yet.`}</div>}
        {(() => {
          const groups = {};
                  ...
                  <div key={item.id} style={{ background: "#fff", borderRadius: 10, padding: "11px 14px", marginBottom: 8, display: "flex", alignItems: "center", gap: 12, border: days !== null && days <= 2 ? `1px solid ${col}66` : "1px solid #ece6da" }}>
                    ...
                    <div style={{ flex: 1, minWidth: 0 }}>
                      <div style={{ display: "flex", alignItems: "baseline", gap: 6 }}>
                        ...
                        {item.quantity && <span style={{ fontSize: 12, color: "#999" }}>{item.quantity}</span>}
                      </div>
...
const labelStyle = { fontSize: 11, color: "#999", display: "block", marginBottom: 4, letterSpacing: 0.5 };
